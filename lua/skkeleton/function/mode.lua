...
function M.abbrev(context)
	...
	s.table = filtered
	s.directInput = true
	require("skkeleton.mode").mode_change(context, "abbrev")
end
...
function M.hirakana(context)
	...
	require("skkeleton.mode").mode_change(context, "hira")
end
...
function M.katakana(context)
	...
	local config = require("skkeleton.config").config
	local hira_to_kata = require("skkeleton.kana.hira_kata").hira_to_kata
...
function M.hankatakana(context)
	...
	local config = require("skkeleton.config").config
	local kana_mod = require("skkeleton.kana")
	local hira_to_hankata = require("skkeleton.kana.hira_hankata").hira_to_hankata
	local mode_mod = require("skkeleton.mode")
	local state_mod = require("skkeleton.state")
	local state = context.state
	if state.mode == "direct" then
...
function M.zenkaku(context)
	...
	state.table = kana_mod.get_kana_table()
	require("skkeleton.mode").mode_change(context, "zenkaku")
end
