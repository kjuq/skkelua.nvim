...
---@param pre_edit skkeleton.PreEdit
---@param kana string
---@param feed string
	...
	if state.mode == "okuriari" and feed == "" then
		if util.char_sub(state.okuriFeed, -1) == "っ" then
			...
			if config.immediatelyOkuriConvert then
				require("skkeleton.function.henkan").henkan_first(context, "")
			end
		else
			require("skkeleton.function.henkan").henkan_first(context, "")
		end
...
---@param context skkeleton.Context
---@param result skkeleton.KanaResult
---@param feed string
...
function M.prefix(context, key)
	...
	then
		...
		state.affix = "prefix"
		require("skkeleton.function.henkan").henkan_first(context, key)
		return
	end
