...
---@class skkeleton.SkkDictionary: skkeleton.Dictionary
local Dictionary = {}
...
function Dictionary:get_completion_result(prefix, feed)
	local candidates = {}
	if feed ~= "" then
		local table_ = require("skkeleton.kana").get_kana_table()
...
---@return skkeleton.SkkDictionary
function Dictionary:load(path, encoding)
	if path:match("%.ya?ml$") then
		error("skkeleton: yaml dictionary is not supported by the lua version: " .. path)
...
function Dictionary:load_structured(jisyo)
	if type(jisyo) ~= "table" or type(jisyo.okuri_ari) ~= "table" or type(jisyo.okuri_nasi) ~= "table" then
		error("skkeleton: invalid dictionary: okuri_ari/okuri_nasi are required")
	end
	local function validate_and_build(record)
		...
		for word, candidates in pairs(record) do
			if type(word) ~= "string" or type(candidates) ~= "table" then
				error("skkeleton: invalid dictionary entry")
			end
			for _, c in ipairs(candidates) do
				if type(c) ~= "string" then
					error("skkeleton: invalid dictionary candidate")
				end
			end
			words[#words + 1] = word
		end
...
function Dictionary:load_json(data)
	...
	if not ok then
		error("skkeleton: failed to parse json dictionary: " .. tostring(jisyo))
	end
...
function Dictionary:load_msgpack(data)
	...
	if not ok then
		error("skkeleton: failed to parse msgpack dictionary: " .. tostring(jisyo))
	end
	self:load_structured(jisyo)
end
...
---@class skkeleton.SkkDictionarySource
local Source = {}
...
function Source:get_dictionaries()
	...
	for _, entry in ipairs(config.globalDictionaries) do
		...
		if ok then
			...
			vim.notify(
				("globalDictionary loading failed\nat %s"):format(path),
