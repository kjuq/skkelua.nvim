...
---@class skkeleton.GoogleJapaneseInput: skkeleton.Dictionary
local Dictionary = {}
...
function Dictionary:get_midashis(prefix)
	...
	if vim.fn.executable("curl") ~= 1 then
		if config.debug then
			vim.print("skkeleton: curl is not available")
		end
		return {}
	end
	...
	local ok, result = pcall(function()
		return vim.system(
			{ "curl", "-s", "--max-time", tostring(TIMEOUT_MS / 1000), url },
			{ text = true }
		):wait(TIMEOUT_MS + 200)
	end)
...
---@class skkeleton.GoogleJapaneseInputSource
local Source = {}
