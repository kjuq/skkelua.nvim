...
---@class skkeleton.UserDictionary: skkeleton.Dictionary
local Dictionary = {}
...
function Dictionary:cache_candidates(prefix, feed)
	...
	local candidates = {}
	if feed ~= "" then
		local table_ = require("skkeleton.kana").get_kana_table(config.kanaTable)
...
function Dictionary:read_file(path, rank_path)
	...
	if not ok or type(rank_data) ~= "table" then
		error("skkeleton: invalid rank file")
	end
...
function Dictionary:write_file(path, rank_path)
	...
	if not f then
		vim.notify(("warning(skkeleton): can't write userDictionary to %s"):format(path), vim.log.levels.WARN)
		error(err)
	end
	...
	if not rf then
		vim.notify(
			("warning(skkeleton): can't write candidate rank data to %s"):format(rank_path),
			vim.log.levels.WARN
		)
		error(rerr)
	end
...
---@class skkeleton.UserDictionarySource
local Source = {}
...
---@return skkeleton.UserDictionary
