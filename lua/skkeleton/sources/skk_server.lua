...
---@class skkeleton.SkkServer: skkeleton.Dictionary
local Dictionary = {}
...
function Dictionary:connect(close)
	...
	if not addr then
		error(("skkeleton: can't resolve host: %s"):format(self.hostname))
	end
	...
	if connect_result ~= "ok" then
		tcp:close()
		error(("skkeleton: can't connect to skk server %s:%d (%s)"):format(
			self.hostname,
			self.port,
...
---@class skkeleton.SkkServerSource
local Source = {}
