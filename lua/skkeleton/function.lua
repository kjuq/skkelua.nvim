...
---@alias skkeleton.Func fun(context: skkeleton.Context, char: string)

---@type table<string, skkeleton.Func>?
local functions_cache = nil
---@type table<string, skkeleton.Func>?
local mode_functions_cache = nil

--- モード切り替え関数のテーブル (modeFunctions 相当)
---@return table<string, skkeleton.Func>
function M.mode_functions()
	if not mode_functions_cache then
		local mode = require("skkeleton.function.mode")
...
function M.functions()
	if not functions_cache then
		...
		local disable = require("skkeleton.function.disable")
