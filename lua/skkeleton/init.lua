...
local function init()
	...
	if config.debug then
		vim.print("skkeleton: initialize")
		vim.print(config)
	end
	...
	store.set_library_initializer(function()
		return require("skkeleton.dictionary").load(config.sources)
	end)

	local group = vim.api.nvim_create_augroup("skkeleton-internal-lua", { clear = true })
	...
	if notation_str == "<cr>" then
		if completed and config.eggLikeNewline then
			if complete_type == "native" then
				return require("skkeleton.notation").notation_to_key["<c-y>"]
...
---@param opts table
---@param vim_status skkeleton.VimStatus
---@return string
local function handle_impl(opts, vim_status)
	...
	if is_truthy(vim_status.completeInfo.pum_visible) then
		...
		if type(handled) == "string" then
			require("skkeleton.mode").initialize_state_with_abbrev(context, { "converter" })
	...
	if opts["function"] and opts["function"] ~= "" then
		local fn = require("skkeleton.function").functions()[opts["function"]]
		if not fn then
		...
		for _, key in ipairs(key_list) do
			require("skkeleton.keymap").handle_key(context, key)
		end
	end
...
local function enable(opts, vim_status)
	...
	if vim.fn.mode() == "R" then
		vim.print("skkeleton doesn't allowed in replace mode")
		return ""
	end
	...
	require("skkeleton.kana").set_current_kana_table(config.kanaTable)
	local context = store.init_context()

	emit_user_autocmd("skkeleton-enable-pre")

	require("skkeleton.option").save_and_set()
	M.map()
	vim.g["skkeleton#enabled"] = true
	local mode_fn = require("skkeleton.function").mode_functions()[store.variables.lastMode]
	if mode_fn then
		mode_fn(context, "")
	end
...
local function disable(opts, vim_status)
	...
	-- Note: plugin/skkeleton.lua で定義している物は opts が空なのでこっちは呼ばない
	if (state.type ~= "input" or state.mode ~= "direct") and is_opts(opts) and vim_status then
		return handle_impl(opts, vim_status)
	end
	require("skkeleton.function.disable").disable(context)
...
local function build_result(result)
	local state = require("skkeleton.store").get_context().state
	local phase
...
function M.map()
	...
	require("skkeleton.map").save(mode)
	...
	for _, c in ipairs(mapped_keys) do
		...
		local plug = vim.fn.maparg(c, mode):match("<Plug>%(skkeleton%-(%a+)%)")
		if plug then
			func = plug
		end
		vim.keymap.set(mode, c, function()
			require("skkeleton").handle(func, { key = k })
		end, {
			buffer = true,
			nowait = true,
			silent = true,
			desc = ("skkeleton %s (%s)"):format(func, k),
		})
	end
end

--- skkeleton のバッファローカルマッピングを全て消す
...
function M.dangerously_clear_buffer_local_mappings()
	...
	for _, mode in ipairs({ "i", "c", "t", "l", "n", "v", "s", "o" }) do
		for _, m in ipairs(vim.api.nvim_buf_get_keymap(bufnr, mode)) do
			...
			if rhs:find("skkeleton", 1, true) or desc:find("skkeleton", 1, true) then
				local ok = pcall(vim.api.nvim_buf_del_keymap, bufnr, mode, m.lhs)
				if not ok then
					vim.notify(
						("[skkeleton.dangerously_clear_buffer_local_mappings] failed: %sunmap <buffer> %s"):format(
							mode,
							m.lhs
						),
...
function M.disable_impl()
	if is_truthy(vim.g["skkeleton#enabled"]) then
		...
		require("skkeleton.map").restore()
		require("skkeleton.option").restore()
...
function M.reset()
	require("skkeleton.store").init_context()
end
...
function M.config(cfg)
	require("skkeleton.config").set_config(cfg)
end
...
---@return skkeleton.ConfigOptions
function M.get_config()
	return require("skkeleton.config").config
end
...
function M.register_keymap(state, key, func_name)
	local notation = require("skkeleton.notation")
	require("skkeleton.keymap").register_key_map(state, notation.normalize(key), func_name)
end
...
function M.register_kanatable(table_name, table_, create)
	require("skkeleton.kana").register_kana_table(table_name, table_, create)
end
...
function M.register_kanatable_file(table_name, path, encoding, create)
	require("skkeleton.kana").load_kana_table_file(table_name, path, encoding or "", create)
end
...
function M.initialize(force)
	...
	require("skkeleton.store").get_library()
end

--- deno_kv データベースは Lua 版では非対応
function M.update_database(_path, _encoding, _force)
	vim.notify(
		"skkeleton: updateDatabase (deno_kv) is not supported by the lua version",
		vim.log.levels.WARN
	)
end
...
function M.get_pre_edit()
	return require("skkeleton.store").get_context():to_string()
end
...
function M.get_pre_edit_length()
	return require("skkeleton.util").char_len(M.get_pre_edit())
end
...
--- 見出しの変換候補を返す
---@param kana string
---@param type_? skkeleton.HenkanType
---@return string[]
...
--- 補完候補を返す
---@return skkeleton.CompletionData
...
function M._reset_for_test()
	initialized = false
	require("skkeleton.store").init_context()
	require("skkeleton.store").init_library()
	require("skkeleton.function.henkan")._reset()
end
