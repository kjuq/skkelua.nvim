...
local validators = {
	...
	kanaTable = function(x)
		...
		local ok = pcall(require("skkeleton.kana").get_kana_table, name)
		if not ok then
...
function M.set_config(new_config)
	if M.config.debug then
		vim.print("skkeleton: new config")
	...
	require("skkeleton.kana").load_kana_table_files(M.config.globalKanaTableFiles)
end
